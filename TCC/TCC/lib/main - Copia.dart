...
class NewsIF extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: FutureBuilder<NoticiaListModel>(
                    future: fetchPost(),
                          ...
                          children: <Widget>[
                            Card(                              
                              ...
                              onTap:(){
                                ...
                                Navigator.push(context,
                                MaterialPageRoute(
                                  builder: (context) => FutureBuilder<NoticiaListModelCompleta>(
                                  future: fetchPost2(i),
...
String data = "";
Future<NoticiaListModelCompleta> fetchPost2(int id) async {
  
  final response = await http.get('http://app.bambui.ifmg.edu.br/integracao/noticia/retornarNoticia?id=${id}');

  if (response.statusCode == 200) {
    return NoticiaListModelCompleta.fromJson(json.decode(response.body));
  } else {
    throw Exception('Falha ao carregar um post');
  }
}
...
class NoticiaListModelCompleta {

  final List<NoticiaCompleta> noticiaListCompleta;

  NoticiaListModelCompleta({this.noticiaListCompleta});

  factory NoticiaListModelCompleta.fromJson(List<dynamic> json) {
    List<NoticiaCompleta> results2 = [];
    for(int i = 0; i<json.length; i++){
      NoticiaCompleta _noticia2 = NoticiaCompleta(id: json[i]['id'], title: json[i]['titulo'], texto: json[i]['texto'], dataHora: json[i]['data_hora_publicacao']);
      results2.add(_noticia2);
    }
    return NoticiaListModelCompleta(noticiaListCompleta: results2);
  }
}
class NoticiaCompleta {
  ...
  NoticiaCompleta({this.id, this.title,this.texto, this.dataHora});
}
